...
@section('content')
    @if (count($data_anggota))
        <div class="alert alert-success mb-4">
            Ditemunkan <strong>{{ count($data_anggota) }}</strong> data dengan kata: <strong>{{ $search }}</strong>
        </div>
            ...
            <tbody>
                @foreach ($data_anggota as $index => $anggota)
                    <tr>
                        ...
                        <td>{{ $anggota->birth_date ? $anggota->birth_date->translatedFormat('d F Y') : 'unavailable' }}
                        </td>
                        <td>
    ...
    @else
        <div class="alert alert-danger mb-4">
            ...
            <a href="/members" class="btn btn-warning">Kembali</a>
        </div>
    @endif
@endsection
